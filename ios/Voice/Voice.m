...
RCT_EXPORT_METHOD(getSupportedLocales:(RCTResponseSenderBlock)callback)
{
    ...
    NSMutableArray *localeArray = [NSMutableArray array];
    
    for (NSLocale *locale in locales) {
        [localeArray addObject:[locale localeIdentifier]];
    }

    callback(@[[NSNull null], localeArray]);
}

// Add a method to check for supported locales
- (BOOL)isSupportedLocale:(NSString *)localeIdentifier {
    for (NSLocale *supportedLocale in [SFSpeechRecognizer supportedLocales]) {
        if ([supportedLocale.localeIdentifier isEqualToString:localeIdentifier]) {
            return YES;
        }
    }
    return NO;
